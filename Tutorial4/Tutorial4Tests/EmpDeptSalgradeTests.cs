...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = (from emp in emps where emp.Job.Equals("SALESMAN") select emp).ToList(); 
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = (from emp in emps
            where emp.DeptNo == 30
            orderby emp.Sal descending select emp).ToList();
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (from emp in emps
            where
                (from dept in depts where dept.Loc.Equals("CHICAGO") select dept.DeptNo)
                .Contains(emp.DeptNo)
            select emp).ToList();
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = from emp in emps
            join dept in depts on emp.DeptNo
                equals dept.DeptNo
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        into counter select new {DeptNo = counter.Key, Count = counter.Count()}); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = from emp in emps
            where emp.Comm is not null select new {emp.EName,emp.Comm}; 
    ...
    {
        var emps = Database.GetEmps();
        var srednia = emps.Average(emp => emp.Sal);

        var result = from emp in emps
            group emp by emp.DeptNo
            into help
            select new { DeptNo = help.Key, AvgSal = srednia }; 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from emp in emps
            where emp.Sal >
                  (from empik in emps
                      where empik.DeptNo == emp.DeptNo
                      select empik.Sal).Average()
            select emp.EName;
